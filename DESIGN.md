# Design Notes

Design rationale per phase, updated as decisions are made.

## Phase 1: Vector and dataset representation

Each vector is D contiguous floats. Distance computation and hyperplane projection both need to walk all D dimensions of one vector together, so the vector itself is the unit that must stay contiguous, not one array per dimension across the dataset.

The dataset is stored as a single contiguous `malloc`'d block of `N * D` floats (row major: vector `i` occupies floats `[i*D, (i+1)*D)`), not as N separately allocated vectors behind an array of pointers. The dataset is built once by the generator and only read afterward, so the per-vector-malloc approach's main advantage (independent resize/free) is not needed. Its costs are real: pointer chasing to reach each vector, unpredictable placement in memory, extra malloc/free overhead. One contiguous block gives cache-friendly sequential access, relevant whenever a leaf bucket or the brute-force baseline scans multiple vectors, at no extra implementation cost over the alternative.

Dimensionality D is a runtime parameter, not a compile-time constant. Phase 6 benchmarks latency across multiple dimensionalities, so it cannot be fixed at build time.

## Phase 1: Synthetic dataset generation

Chose a low-rank shared-latent-factor model over generating a full covariance matrix and applying a Cholesky decomposition. Without real MFCC statistics to calibrate a covariance matrix against, Cholesky's extra statistical rigor does not translate into more realism here, while it adds implementation and numerical-stability risk (an arbitrarily chosen covariance matrix is not guaranteed positive semi-definite) to a supporting component rather than the core deliverable.

Mechanism: draw a small number of shared Gaussian latent factors (via a Box-Muller uniform-to-Gaussian transform), combine them through weights into each of the D output dimensions, add independent per-dimension noise. Because multiple output dimensions share the same underlying factors, they come out correlated. This gives the dataset the kind of low-dimensional manifold structure that random projection trees are built to exploit (Dasgupta and Freund, below), rather than uncorrelated high-dimensional noise.

Per-dimension variance decay (mimicking how real MFCC coefficient magnitude falls off with index) applied via a scaling factor decreasing with dimension index.

Noise is non-isotropic: each output dimension gets its own variance from the decay schedule rather than one shared variance across all dimensions. The isotropic case (equal variance in every dimension) is the special condition under which this model's maximum-likelihood solution reduces to PCA, and using it would also conflict with the per-dimension variance decay already decided above.

Exact factor count, weighting scheme, and decay schedule are implementation choices made while writing the generator.

## Phase 1: Implementation (Dataset struct)

`Dataset` holds a `float *data` pointer to the contiguous buffer, plus `size_t n` and `size_t dim`. `size_t` is used instead of `int`: it is unsigned (a negative count is not representable) and matches the type `malloc` and `sizeof` use.

`dataset_create` allocates `n * dim` floats and returns the struct by value. It does not check for integer overflow in `n * dim * sizeof(float)`. Not a real risk at this project's scale, left unchecked deliberately.

`dataset_free` frees the buffer and zeroes the struct's fields, so a stale `Dataset` fails loudly (via `dataset_at`'s assert, or a safe no-op `free(NULL)`) rather than causing silent memory corruption.

`dataset_at` returns a pointer to vector `i`'s data via `data + i * dim`, with an `assert(i < n)` bounds check. Asserts are compiled out if `NDEBUG` is defined. The project's Makefile does not define it in either build target, so the check stays active in both.

## Phase 1: Implementation (random utilities)

`include/random_utils.h` and `src/random_utils.c` provide `uniform_random` and `gaussian_random`, used by the generator for both the factor draws and the per-dimension noise.

`gaussian_random` uses the Box-Muller transform: two independent uniform values in (0, 1) produce two independent standard normal values. Only one of the two is used per call, the second discarded. A known inefficiency, acceptable at this project's scale.

Both functions return `double`, not `float`. The intermediate math (`log`, `sqrt`, `cos`) is more numerically stable in double precision. The generator casts down to `float` only when writing the final value into the `Dataset` buffer.

`PI` is defined locally rather than using `M_PI`. `M_PI` is a POSIX/glibc extension, not part of ISO C, and is not guaranteed to be declared under the project's `-std=c11` strict mode.

The Makefile's `LDFLAGS` now includes `-lm`, required to link `sqrt`, `log`, and `cos` from the math library.

## Phase 1: Implementation (Generator)

`include/generator.h` and `src/generator.c` provide `generate_dataset(n, dim, k)`, implementing the shared-latent-factor model above.

The loading matrix L (dim by k) is stored the same way the Dataset itself is: one flat `malloc`'d block of `dim * k` doubles, indexed by hand as `loadings[j * k + f]`, rather than an array of pointers. Same reasoning as the Dataset struct: one allocation, contiguous, no pointer chasing.

L and the per-dimension decay schedule are computed once, before any vector is generated, and shared by all n vectors. For each vector: a fresh k-length factor draw, a matrix-vector product against L giving the raw dim-length signal, fresh independent noise added to it, and the sum scaled by the decay schedule.

The decay multiplication is a single trick doing two jobs at once. Rather than tracking a separate noise-variance schedule in addition to the magnitude-decay schedule, the same per-dimension decay factor is applied at the end, to the signal and the noise together. Because both are scaled by the same shrinking factor for a given dimension, the noise ends up smaller in exactly the dimensions where the signal is smaller, which is the non-isotropic property decided above, without a second array to maintain. This final multiplication is an addition on top of the textbook factor analysis equation X = LF + ε, not part of the original model.

The nested loops that build each vector run in O(n * dim * k) total: n vectors, each needing dim output values, each of which requires summing k terms. This is the minimum work the computation can take, not an accidental complexity blowup; producing n * dim numbers that each require a k-term sum cannot be done in less than n * dim * k operations.

Neither this module nor random_utils calls `srand()`. Seeding is left to the caller (main, once wired up), since a library function reseeding on every call would be wrong, and calling it more than once per program run would defeat its purpose.

## Phase 1: Empirical verification

`tests/test_generator.c` checks the two structural properties the shared-latent-factor design claims to produce: decaying per-dimension variance and nonzero correlation between dimensions.

Variance measured across a generated dataset (D = 20) fell from 3.455 at dimension 0 to 0.013 at dimension 19, a drop of about 270x. This matches the decay schedule's prediction: variance scales with the square of the per-dimension decay factor, and the ratio of the last decay factor to the first, squared, comes out close to the measured ratio. The trend is not strictly monotonic (dimension 2 measured higher than dimensions 0 and 1, for instance), which is expected: each dimension's row in the loading matrix L is an independent random draw, so the smooth decay envelope carries per-dimension noise on top of it.

The correlation matrix over the first six dimensions showed a diagonal of exactly 1.000 (each dimension perfectly correlated with itself, confirming the correlation computation itself) and clearly nonzero off-diagonal entries in both directions, for example 0.632 between dimensions 0 and 3, and -0.605 between dimensions 1 and 2. Negative correlations are expected under this model: two dimensions sharing a latent factor with opposite-signed weights on it move in opposite directions, and L's entries are zero-centered, so this happens routinely. A few pairs (dimension 0 and 5, at 0.027) came out close to zero, also expected since not every dimension pair shares much factor weight by chance.

Both properties together confirm the generator produces data with genuine low-dimensional structure to exploit, rather than uncorrelated high-dimensional noise. This is the premise the rest of the project, tree splits that find structure-aligned hyperplanes, depends on. Phase 1 is closed out on this basis.

## Sources consulted (Phase 1)

- Erik Bernhardsson, "Nearest neighbor methods and vector models", part 1: https://erikbern.com/2015/09/24/nearest-neighbor-methods-vector-models-part-1.html
- Erik Bernhardsson, part 2: https://erikbern.com/2015/10/01/nearest-neighbors-and-vector-models-part-2-how-to-search-in-high-dimensional-spaces.html
- Dasgupta and Freund, "Random projection trees and low dimensional manifolds" (STOC 2008): https://cseweb.ucsd.edu/~dasgupta/papers/rptree-stoc.pdf
- spotify/annoy source: https://github.com/spotify/annoy
- MFCC tutorial (Practical Cryptography): http://practicalcryptography.com/miscellaneous/machine-learning/guide-mel-frequency-cepstral-coefficients-mfccs/
- Cepstrum, Wikipedia: https://en.wikipedia.org/wiki/Cepstrum
- Array of Structs and Struct of Arrays: https://hwisnu.bearblog.dev/array-of-structs-and-struct-of-arrays/
- FAISS IndexFlatL2 docs: https://faiss.ai/cpp_api/struct/structfaiss_1_1IndexFlatL2.html
- Pinecone, "Nearest Neighbor Indexes for Similarity Search": https://www.pinecone.io/learn/series/faiss/vector-indexes/
- Box-Muller transform, Wikipedia: https://en.wikipedia.org/wiki/Box%E2%80%93Muller_transform
- Box Muller Transform, Algorithm Archive: https://www.algorithm-archive.org/contents/box_muller/box_muller.html
- Cholesky decomposition intuition: https://alexander-pastukhov.github.io/notes-on-statistics/advanced-04-cholesky.html
- The Math Behind the Curse of Dimensionality: https://towardsdatascience.com/the-math-behind-the-curse-of-dimensionality-cf8780307d74/
- Factor analysis, Wikipedia: https://en.wikipedia.org/wiki/Factor_analysis
- FactorAnalysis, scikit-learn documentation: https://scikit-learn.org/stable/modules/generated/sklearn.decomposition.FactorAnalysis.html
- Random Projection, scikit-learn user guide: https://scikit-learn.org/stable/modules/random_projection.html

## Phase 2: Node representation

A random projection tree node is one of two kinds: an internal node, which only decides which of two children a point belongs to, and a leaf, which holds the actual points that landed in that region. `RPNode` represents both with a single struct and an `is_leaf` tag rather than two separate struct types (one for internal nodes, one for leaves) behind a discriminated pointer or a union. All fields for both cases exist on every node, wasting some memory (a leaf carries unused `normal`/`threshold`/`left`/`right` fields, an internal node carries unused `indices`/`count`), which a union would avoid. That waste is accepted for now: compact, cache-aware node layout is its own later phase (roadmap item 7), and doing it now would mean optimizing a data structure whose access patterns have not been measured yet.

## Phase 2: Implementation (RPNode struct)

`is_leaf` is a plain `int`, not `bool` from `<stdbool.h>`. Neither `Dataset`, `random_utils`, nor `generator` use `<stdbool.h>` anywhere in the project so far, so `int` keeps the new struct consistent with the rest of the codebase's existing style rather than introducing a second convention for the same 0/1 flag.

`RPNode`'s internal-node fields are `float *normal` and `float threshold` (the hyperplane's coefficients, derived from two random pivot points, see the split-rule algebra worked out in chat) plus `struct RPNode *left` and `struct RPNode *right`. Its leaf fields are `size_t *indices` and `size_t count`.

The struct is declared as `typedef struct RPNode { ... } RPNode;`, naming the struct twice: `RPNode` immediately after `struct` is the struct tag, `RPNode` after the closing brace is the typedef alias. The tag is required here specifically because the struct is self-referential (`left` and `right` are pointers to the same struct type), and inside the struct body the typedef name does not exist yet, only the tag does. This is the standard C idiom for any self-referential struct (tree nodes, linked list nodes).

## Phase 2: Implementation (RPTree struct)

The recursive build (not yet written) partitions one flat array of point indices in place: the root call gets the full array, and each split hands its two children pointers into two contiguous sub-ranges of that same array, not separately allocated copies. A leaf's `indices` pointer is therefore frequently not the address `malloc` originally returned, it can be offset into the middle of that block. Freeing such a pointer directly is undefined behavior.

`RPTree` exists to make this safe: it wraps the tree's `root` together with the single flat `indices` array the whole tree is built from, and owns that array exclusively. The array is allocated once, when the tree is built, and freed exactly once, when the tree is freed. Individual `RPNode` leaves never own or free their `indices` pointer, they only reference into the array `RPTree` owns.

## Phase 2: Implementation (split rule and recursive build)

`choose_split` implements the derived split rule directly: two distinct random positions are drawn from the current index range via `uniform_random`, the vectors at those positions become pivots A and B, and `normal = B - A`, `threshold = (‖B‖² - ‖A‖²) / 2` are computed component by component, matching `x·n < t` from the algebra worked out for "closer to A than to B". The squared-length accumulations are done in `double` even though vectors are stored as `float`, only cast back to `float` for the final threshold, avoiding precision loss when summing many small floats across `dim` dimensions (same reasoning as the generator's use of `double` intermediates).

`partition_indices` rearranges `indices[0..count)` in place using a single two-pointer pass (the same technique as the partition step in quicksort): a point already on its correct side advances a `left` pointer, otherwise it is swapped toward the end of the range and a `right` pointer shrinks. Every point is visited exactly once, no second array is allocated.

`build_recursive` checks the two stopping conditions (`count <= max_leaf_size` or `depth >= max_depth`) before doing any split work, allocates a leaf directly if either holds, and otherwise computes the split, partitions, and recurses on the two resulting contiguous sub-ranges of the same shared indices array (`indices` and `indices + split`), consistent with the `RPTree` ownership design documented above. No special case was added for a split that fails to separate the group at all (`split == 0` or `split == count`, the degenerate case discussed for duplicate-heavy data): the recursion simply continues, depth increases, and `max_depth` is the backstop that eventually forces a leaf.

`rptree_build` is the public entry point: allocates the single flat `indices` array (one slot per point, initialized to `0..n-1`), then calls `build_recursive` on the whole array at depth 0. `rptree_free` is its inverse, recursively freeing each internal node's `normal` array and every node struct, then freeing the shared `indices` array exactly once at the end, never per leaf.

## Phase 2: Empirical verification (tree build)

`tests/test_tree.c` builds a tree over the same shared-latent-factor dataset used in Phase 1's verification (n = 2000, dim = 20, k = 4, seed 42), with `max_leaf_size = 20` and `max_depth = 20`, and checks two invariants a correct build must satisfy plus one structural report.

Partition correctness: every one of the 2000 points appeared in exactly one leaf (0 missing, 0 duplicated across leaves), confirming `partition_indices` and the recursive split neither lose nor double count points while rearranging the shared indices array in place.

Split self-consistency: re-deriving, independently for each of the 2000 points, which leaf the tree's own stored `normal`/`threshold` values say it belongs in (descending from the root by `dot(point, normal) < threshold` at each internal node) landed on the same leaf `build_recursive` actually placed it in, for all 2000 points, 0 mismatches. This is the same decision rule Phase 3's search will walk, so it also confirms the mechanism search will depend on is internally consistent.

Structurally, the tree built 168 leaves, sizes ranging from 1 to 20 (the `max_leaf_size` cap), averaging 11.9, and reached a maximum depth of 13 against a cap of 20, well short of the degenerate case discussed for duplicate-heavy data. This is consistent with Phase 1's empirical finding that the dataset carries genuine low-dimensional structure (nonzero cross-dimension correlation): the random hyperplanes found real separations in most splits rather than repeatedly failing to divide the data.

Both checks and the debug build (`-fsanitize=address,undefined`) running clean on `test_tree` together close out Phase 2's build correctness: the tree partitions every point exactly once, its own split rule is self-consistent, and its manual memory management (the `RPTree`-owned indices array, per-node `normal` allocations) has no detected leaks, invalid frees, or undefined behavior.


## Phase 3: Search strategy

A strictly defeatist search (one root-to-leaf walk per tree, following `dot(point, normal) < threshold` at each internal node exactly as `find_leaf` already does) commits irreversibly to the split's decision at every internal node. Because the split hyperplane is chosen from two random pivot points rather than an exact optimal cut, a point lying close to the hyperplane can fall on the "wrong" side of a given split while its true nearest neighbors sit just across the boundary, and a single fixed path has no way to recover from that.

Reading Annoy's `get_all_nns_by_vector` (`src/annoylib.h`, spotify/annoy) and Erik Bernhardsson's writeup of the same design (both already in Phase 1's sources list) showed the alternative: a best-first search driven by a single priority queue, seeded with the tree's root, where descending into an internal node pushes the side not taken back onto the queue instead of discarding it. The queue is popped repeatedly, not just walked once, each pop either yielding leaf points as candidates or expanding an internal node further, until a caller-supplied budget of candidate points is reached or the queue empties. This generalizes directly to a forest by seeding the same queue with one root per tree instead of one, so the mechanism built now is not single-tree-specific machinery to be replaced later, it is the same search a forest search will use, just seeded with more roots.

The priority attached to each queued node is the minimum absolute margin (`|dot(query, normal) - threshold|`) seen along the path leading to it, not the margin of its own split alone. A node reached through one ambiguous split and several confident ones afterward is only as trustworthy as the ambiguous one; taking the running minimum instead of the most recent margin prevents a later confident split from erasing the doubt an earlier one introduced. Smaller priority means the branch is more plausible and should be explored sooner, so the queue is a min-heap.

## Phase 3: Implementation (priority queue)

`include/pqueue.h` and `src/pqueue.c` implement the min-heap the search above depends on. C has no built-in priority queue or heap container, that is a C++-only convenience, so this is a from-scratch array-backed binary heap over `PQEntry { RPNode *node; float priority; }` pairs, with the standard sift-up on insert and sift-down on removal.

`pqueue_create` returns a `PQueue` by value with failure signaled through `entries == NULL`, matching `dataset_create` and `rptree_build`'s existing convention rather than an out-parameter and status code. `pqueue_push` and `pqueue_pop` return explicit `int` status instead, consistent with the codebase's existing habit of checking every allocation rather than assuming success.

The backing array grows by doubling (via `realloc`) rather than being sized to a fixed, precomputed capacity. A fixed bound is computable, live entries at any moment track `max_depth`, since exactly one deferred child is queued per level along the currently followed path, but getting that bound wrong in either direction is a real risk: too generous wastes memory, too conservative silently fails to queue a branch the search still needed to explore, corrupting correctness rather than just speed. Growing on demand removes that risk at a small, one-time reallocation cost.

## Phase 3: Implementation (search function)

`rptree_search` and its private helper `collect_candidates` (`src/tree.c`) implement the search strategy above. `collect_candidates` is the traversal: starting from the root, it repeatedly walks directly into the "near" child at each internal node, the side selected by the same `dot(query, normal) < threshold` test used at build time, computed through a new shared `dot_product` helper also used by `partition_indices`, so build-time and query-time routing are the same code, not two hand-written copies of the same loop, pushing the "far" child onto the priority queue with the running-minimum margin as priority, until a leaf is reached. Every leaf's points are added to a growable candidate list (the same doubling-`realloc` strategy as the queue itself), and the walk resumes from the best remaining queue entry until `search_budget` candidates have been collected or the queue is empty.

`rptree_search` is the public entry point: it calls `collect_candidates` for the raw candidate pool, scores every candidate by squared Euclidean distance to the query (squared rather than true distance, since ranking only needs relative order and `sqrt` is redundant work for every candidate), sorts by distance with `qsort`, and returns the closest `k` as an `RPSearchResult` (indices and distances, by value, `indices == NULL` signaling allocation failure, following the same convention as `Dataset` and `RPTree`). `rptree_search_free` mirrors `dataset_free`/`rptree_free`.

The design keeps `near` off the queue entirely rather than pushing both children, as Annoy's actual implementation does: only the untaken side is ever queued, so the running-minimum margin unambiguously orders how ambiguous the decision that deferred each branch was, without needing a signed value to distinguish taken from untaken siblings, since the taken side is never a competing queue entry. This is a correct, standard approach for one tree (best-bin-first search), checked directly against Annoy's real source rather than assumed. The tradeoff, found while checking that source and not yet resolved: Annoy's push-both design gives every tree root a fair first look before any one tree's greedy path is allowed to run deep, because a fresh root's initial priority survives only until its own first split; this design has no such fairness property once multiple roots would share one queue, since the greedy walk down `near` runs uninterrupted to a leaf before the queue is ever consulted. Not a problem for a single tree, deferred until multiple tree roots are actually combined.

Also accepted as-is: `build_recursive`'s two recursive calls store the result directly into `node->left`/`node->right` without checking for `NULL`. A `malloc` failure at any point during the recursion therefore produces a tree with a null child pointer rather than a reported error, and any later code that walks the tree assuming every internal node has two live children, including this search, would crash on that pointer rather than failing cleanly. Not fixed for now: propagating an allocation failure up through a partially built recursive structure means every caller in the chain needs to free the already-built partial subtree and needs its own failure path, disproportionate work relative to how likely a `malloc` failure is at this project's data scale. Documented here as a known, accepted gap.

## Phase 3: Empirical verification (search)

`tests/test_search.c` builds the same tree as Phase 2's verification (same dataset, seed 42, n = 2000, dim = 20, k = 4, `max_leaf_size = 20`, `max_depth = 20`) and checks three properties.

Self-match: querying with each of the 2000 points' own vectors returns that same point as its own nearest neighbor at exactly distance 0, for all 2000 points, 0 mismatches. This holds by construction, not luck: since the query-time routing test is now the identical `dot_product` function build-time partitioning uses, the greedy walk for a point already in the dataset always reaches the exact leaf that partitioning placed it in, on the very first descent, regardless of `search_budget`.

Sorted order: a 10-result query's returned distances were non-decreasing, 0 out-of-order adjacent pairs.

Recall@5: over 50 fresh query points drawn from the same generator (not points already in the tree), with `search_budget = 200` (10% of the 2000-point dataset), the tree's approximate top-5 matched a brute-force exact top-5 (linear scan, computed independently inside the test file) at 0.612 average recall. Expected to be well under 1.0 for a single tree at this budget: recall approaching 1.0 for this kind of search depends on combining multiple independently-split trees, not yet built.

Also verified clean under `-fsanitize=address,undefined` (no leaks, invalid frees, or undefined behavior), after fixing a missing `free` on the queue's allocation-failure path in `collect_candidates` found during review.

## Phase 4: Multi-tree search strategy

A forest needs a search that spans several independently built trees and decides, at every step, which tree's next branch is worth spending the search budget on. Two naive options were ruled out first. Running the existing single-tree search separately on each tree and merging results at the end spends the budget evenly across trees regardless of which tree's structure actually looks promising for this particular query. Extending the current walk-near-directly design to several roots is worse: whichever root is processed first runs its own greedy descent, potentially the tree's full depth (13 levels, per Phase 2's build stats) deep, before any other tree's root is examined even once.

Rereading Annoy's actual search function at the source level (`get_all_nns`, `src/annoylib.h`, spotify/annoy) and the original paper its design descends from, Beis and Lowe's best-bin-first search (CVPR 1997, full priority-queue search across several independently built trees, kd-trees in their case) confirmed the same pattern in both: every child of an internal node is pushed onto the queue, neither one is ever walked into directly. A queue pop then always compares "the best next branch across every tree," not "the best next branch within whichever tree already has a head start."

This project's existing `collect_candidates` (`src/tree.c`) already computes the right quantity for this. `priority`, the running minimum of `|margin|` along the current path, is updated before the near/far branch is even chosen, so it already applies equally to both children at that node. Generalizing to push-both needs no new priority formula, only pushing both children with that already-computed value instead of pushing one and walking into the other directly.

Seeding follows the same principle rather than needing a special case: each tree's root is evaluated once, its own split's margin computed and folded into a priority that starts at `FLT_MAX`, exactly as the current single-tree code already does for its one root, before anything is placed in the shared queue. Every entry that ever enters the queue, from any tree, therefore already carries a real, finite priority.

The existing sort-and-skip-duplicate step used to deduplicate a single tree's candidate list is unchanged for the forest case. It now runs over a list drawn from several trees' leaves and does not distinguish an index repeated from the same tree from one repeated across two different trees.

Queue growth was checked before settling on this design. Each internal-node expansion changes the queue's size by a net +1 (one entry popped, two pushed), since only one node, the current best across the whole queue, is expanded per step; this is best-first search, not a per-level expansion of an entire tree at once. Every node enters the queue at most once over its lifetime, pushed exactly once by its parent when the parent is expanded, so the total entries that can ever be pushed during one query are bounded by the number of internal nodes actually visited, itself governed in practice by `search_budget`, and in the extreme by the whole forest's total internal-node count, a modest, already-measured quantity (roughly 167 internal nodes for one 2000-point tree at `max_leaf_size = 20`, per Phase 2's verification).

Not yet implemented. `collect_candidates` still has the single-root walk-near-directly version described in Phase 3's sections above. This records the settled design ahead of writing the forest search code, the same pattern Phase 3 followed for its own search strategy.

## Phase 4: Implementation (PCG32 random number generator)

A forest needs each tree built with its own independent, reproducible source of randomness for `choose_split`'s pivot picks. `random_utils.c` previously drew from libc's global `rand()`/`RAND_MAX`, a gap identified during Phase 3's external-review verification: weak statistical quality for anything Monte-Carlo-like, and no way to give two trees separate, non-overlapping streams.

Replaced with PCG32 (O'Neill, pcg-random.org), chosen over xoshiro256++ (Blackman and Vigna), both explicit-state generators considered. PCG32's state is two 64-bit integers against xoshiro256++'s four, and its reference implementation is a few lines, where xoshiro256++ needs a separate SplitMix64 step just to expand a single seed into full starting state. xoshiro256++ is generally faster and statistically stronger under heavy use, more than this project's data scale requires.

`include/random_utils.h`/`src/random_utils.c` now center on a `Pcg32State` struct (`state`, `inc`), matching PCG's own minimal reference implementation for `pcg32_seed`/`pcg32_random`. `uniform_random`/`gaussian_random` keep their existing behavior (`gaussian_random`'s Box-Muller math is untouched) but now take a `Pcg32State*` parameter instead of reading implicit global state. `uniform_random`'s normalizing constant changed from `RAND_MAX + 1.0` (platform-dependent under `rand()`) to a fixed `4294967296.0` (2^32), matching `pcg32_random`'s guaranteed full 32-bit unsigned output range, a portable constant rather than an implementation-defined one.

`pcg32_seed` takes two values, `initstate` and `initseq`. `initseq` selects an independent output stream: two generators seeded from the same `initstate` but a different `initseq` produce unrelated, non-overlapping sequences. This is the mechanism a forest's build step will use later to give each tree its own stream from a single base seed, rather than needing unrelated seed values per tree.

Committed (`93c5165`). Not yet done: `tree.c`'s `choose_split`/`rptree_build` and `generator.c` still call the old zero-argument `uniform_random()`/`gaussian_random()`, so the project does not currently compile. Updating those call sites to take a `Pcg32State*`, and deciding how the not-yet-written `rpforest_build` assigns each tree its `initseq`, is the next unit of work.

## Sources consulted (Phase 4)

- Beis and Lowe, "Shape Indexing Using Approximate Nearest-Neighbour Search in High-Dimensional Spaces" (CVPR 1997): https://www.cs.ubc.ca/~lowe/papers/cvpr97.pdf
- PCG, A Family of Better Random Number Generators (Melissa O'Neill), reference implementation and docs: https://www.pcg-random.org/
- xoshiro256++ (David Blackman and Sebastiano Vigna), considered, not chosen: https://prng.di.unimi.it/

## Phase 4: Test suite rebuilt under PCG32

Once `random_utils`, `tree.c`, and `generator.c` all took an explicit `Pcg32State` instead of reading implicit global `rand()` state, the three test files' `srand(42)` calls became inert, nothing in the codebase reads libc's global RNG anymore, and their `generate_dataset`/`rptree_build` calls needed the new seed arguments just to compile.

All three tests keep `initstate = 42` as the recognizable constant carried over from the old `srand(42)` convention, but each independent randomness need within a test gets its own `initseq`, exactly the mechanism `pcg32_seed` exists for: dataset generation uses `initseq = 1`, tree construction uses `initseq = 2`. `test_search.c` generates two separate datasets, the main 2000-point dataset and a second batch of 50 fresh query points meant to be independent of it. Giving both the same `initseq` would have restarted the exact same stream for the second call, making the queries' per-dimension loadings and first rows' factors identical to the main dataset's, defeating the point of testing against genuinely fresh queries. Query generation uses `initseq = 3` to keep it a separate stream.

This is a full rebuild under a different, unrelated RNG algorithm, not a bug fix, so the exact figures differ from the `rand()`-based numbers recorded in Phase 2's and Phase 3's "Empirical verification" sections above. Those are left untouched as a historical record of that earlier build; this project's documentation policy keeps DESIGN.md as the record of what actually happened at each point in time, while README (Phase 4 content not yet written) is what carries the current, accurate description forward.

Same dataset/tree parameters as before (n = 2000, dim = 20, k = 4, `max_leaf_size = 20`, `max_depth = 20`): 167 leaves (previously 168), sizes 1 to 20, average 11.98 (previously 11.9), max depth reached 13 (same as before), 0 points missing or duplicated across leaves, 0 split-consistency mismatches. Search: 0 self-match mismatches across all 2000 points, 0 out-of-order result pairs, recall@5 over 50 fresh queries at `search_budget = 200` averaged 0.648 (previously 0.612), same qualitative conclusion as Phase 3, well under 1.0 for a single tree at this budget. Verified clean under `-fsanitize=address,undefined` as well; both builds produced identical numbers, expected, nothing here depends on optimization level.

## Phase 4: Multi-tree search — design correction (push-both retracted, walk-near-per-root restored)

Implementing "push both children onto the shared queue" as settled in the section above surfaced two problems empirically, not by static review. First, doing so straightforwardly (a single priority value pushed identically to both children) discards the routing signal, `margin`'s sign, entirely and silently: pop order stops depending on the query at all. This was caught only after typing the code and running `test_search.c`: self-match failed for 1946/2000 points, nowhere near the expected 0.

A first correction attempt kept pushing both children but gave `near` a fixed low priority (`0.0f`) so it always won ties, leaving `far`'s existing `min(incoming, abs_margin)` formula unchanged. That made results worse (2000/2000 self-match failures), not better, and the reason is structural, not a further tuning problem: any node reached via a near-priority-`0.0f` edge propagates `incoming_priority = 0.0f` into its own children's `far` computation, and since `abs_margin >= 0` always, `min(0.0f, abs_margin) = 0.0f` unconditionally. Every far branch below the first near-hop collapses to priority 0 too, one level after the fix is first applied, erasing the ranking mechanism even faster than the original bug did. This isn't specific to the constant 0: any fixed value propagated forward through `near` poisons every descendant `far` computation via the same `min` identity, and propagating `incoming_priority` unchanged into `near` instead reintroduces exactly the `FLT_MAX`-starves-the-first-level problem the root-seeding design already avoided (see "Sourcing policy" in WORKFLOW.md). There is no single-number-per-queue-entry scheme under which pushing `near` through the same shared queue and folding it via `min` stays correct.

The corrected design retracts push-both and returns to walking `near` directly, as Phase 3's single-tree search already did, but per tree root during seeding rather than once globally: `collect_candidates` walks each root's `near` chain directly to a leaf, pushing only the `far` sibling at each step onto the one shared queue, with priority equal to the running minimum of `|margin|` crossed so far (unchanged formula, now only ever applied to `far`). Backtracking (popping a queued `far` branch once its priority makes it the best available) walks that branch's own `near` chain the same way, queuing whatever new `far` siblings it finds along the way. This still satisfies the actual goal behind combining multiple trees into one search: every tree's discovered `far` branches sit together in one queue, competing for the shared `search_budget` on equal footing; only the free, no-uncertainty continuation (`near`) is taken outside that competition. This matches Annoy's/Beis and Lowe's own design, which the section above already confirmed pushes every child onto the queue; that reading turned out to mean every far child accumulated across the whole search, never the near child of whichever node is currently being expanded.

## Phase 4: Implementation (multi-tree search)

`collect_candidates` (`src/tree.c`) now takes `RPNode *const *roots, size_t num_roots` instead of a single tree. A static `append_leaf` helper grows the shared candidate array and copies a leaf's indices in (factored out since both the seeding step and the backtracking step need it). A static `walk_near` helper replaces the single-tree version's inline loop: given a starting node and a `bound` (the incoming running-minimum priority), it walks directly into `near` at each internal node, pushing `far` onto the queue with priority `min(bound, abs_margin)` at each step, and appends the leaf it eventually reaches. `collect_candidates` calls `walk_near` once per root with `bound = FLT_MAX` to seed the queue (mirroring the single-root case: `FLT_MAX` is never stored as an actual queue entry, only ever folded through `min` on its first real use), then repeatedly pops the queue's best `far` branch and calls `walk_near` again on it, until `search_budget` candidates are collected or the queue empties.

A public `rptree_search_multi(RPNode *const *roots, size_t num_roots, ...)` wraps `collect_candidates`, then deduplicates (points reachable from more than one tree can appear twice), scores by squared distance, sorts, and truncates to `k`, structurally unchanged from Phase 3's single-tree version. The existing public `rptree_search` is now a thin wrapper calling `rptree_search_multi(&tree->root, 1, ...)`, preserving its own null-tree failure contract (`collect_candidates` itself silently skips a null root, since a forest search tolerates some trees failing to build; a direct single-tree call should not).

One implementation bug found and fixed along the way, unrelated to the near/far design question above: the dedup step's "keep the first of each run of duplicates" loop mistakenly wrote `candidates[candidate_count++]` instead of `candidates[unique_count++]`, both writing past the array's allocated bound (since `candidate_count` was also the loop's own upper bound, growing it while iterating) and leaving `unique_count` permanently 0. This produced the segfault that surfaced before the design bug above was even reachable; found by reading the typed copy directly rather than by review of the design.

## Phase 4: Empirical verification (multi-tree search machinery, single tree)

`tests/test_search.c` unchanged from the Phase 4 PCG32 rebuild (`rptree_search`'s public signature didn't change). Same dataset/tree as before (n = 2000, dim = 20, `max_leaf_size = 20`, `max_depth = 20`, PCG32 `initstate = 42`). Self-match: 0 mismatches across all 2000 points (the rewrite reproduces the same guarantee Phase 3 had: the query's own leaf is always reached deterministically, regardless of `search_budget`). Sorted order: 0 out-of-order pairs. Recall@5 over 50 fresh queries at `search_budget = 200`: 0.624, consistent with the Phase 3/PCG32 figures of 0.612/0.648 (same order of magnitude; the small variation is expected from the walk_near refactor changing exactly which far branches get explored within the same budget). This exercises `rptree_search_multi` with `num_roots = 1` only; the actual forest case (`num_roots > 1`, testing whether recall improves once multiple trees combine) is not yet tested, `rpforest_search` doesn't exist yet.

## Phase 4: Empirical verification (forest search)

`tests/test_forest.c` builds an 8-tree `RPForest` over the same dataset as `tests/test_search.c` (n = 2000, dim = 20, `max_leaf_size = 20`, `max_depth = 20`, PCG32 `initstate = 42`, dataset `initseq = 1`), each tree getting its own PCG32 stream via `rpforest_build`'s per-tree `initseq`. Runs the same three checks as the single-tree test, against `rpforest_search` instead of `rptree_search`, using the same fresh 50-query batch (`initseq = 3`) for a direct comparison.

Self-match: 0 mismatches across all 2000 points, for the same reason as the single-tree case (every tree in the forest independently reaches the exact leaf its own build placed the query point in), now holding across every tree simultaneously. Sorted order: 0 out-of-order pairs. Recall@5 at `search_budget = 200`: 0.868, against the single tree's 0.624 on the identical dataset and queries. This is the actual point of building a forest: combining 8 trees' independently-random splits at the same search budget raised recall by about 24 percentage points, confirming the multi-tree search machinery (per-root `walk_near` seeding, one shared queue of `far` branches pooled across all 8 trees, `search_budget` spent competitively rather than divided evenly per tree) is doing what it was built for, not just passing the self-match/sorted-order sanity checks a broken-but-plausible implementation could also pass.

Verified clean under `-fsanitize=address,undefined` as well (`make clean && make CFLAGS="-std=c11 -Wall -Wextra -Wpedantic -g -O0 -fsanitize=address,undefined" test`); identical numbers to the optimized build, as expected.
